...
from modules.modules import VAE, to_scalar
...
def train(data_loader, model, clfy, optimizer, args, writer=None, loss_fn=None):
    ...
    for images, lables in tqdm(data_loader, total=len(data_loader)):
        ...
        optimizer.zero_grad()
        x_tilde, kl_div, sample = model(images)
        # print(x_tilde.shape)
        ...
        loss = loss_recons + loss_kl + args.gamma * loss_pred
        loss.backward()

        if writer is not None:
            # Logs
            writer.add_scalar('loss/train/reconstruction', loss_recons.item(), args.steps)
            writer.add_scalar('loss/train/kl_div', loss_kl.item(), args.steps)
...
def test(data_loader, model, clfy, args, writer=None, loss_fn=None):
    ...
    if writer is not None:
        ...
        writer.add_scalar('loss/test/kl_div', loss_kl.item(), args.steps)
...
def main(args):
    ...
    model = VAE(num_channels, args.hidden_size, args.prior, pred=True).to(args.device)
